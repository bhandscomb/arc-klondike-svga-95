...
#
#
# Screen code for VESA-compatible SuperVGA boards for Klondike 95
#
#
# DOS INTERRUPTS USED
#
# INT 10h Function AH=4Fh SubFunction AL=00h --- Return SVGA Information
# INT 10h Function AH=4Fh SubFunction AL=01h --- Return SVGA Mode Information
# INT 10h Function AH=4Fh SubFunction AL=02h --- Set SVGA Mode
# INT 10h Function AH=4Fh SubFunction AL=03h --- Return SVGA Mode
# INT 10h Function AH=4Fh SubFunction AL=05h --- Video Memory Window Control
#
#############################################################################
*/
...
/* source control - VESACOLOUR_INLINE turns VesaColour() func into macro */
#define VESACOLOUR_NOTINLINE
...
WORD VesaInit(void) {
  ...
  regs.x.ax=0x4f02;
  regs.x.bx=VIDEO_MODE|(vbe==2?0x4000:0x0000);
  __dpmi_int(0x10,&regs);
  if((regs.h.al!=0x4f)||(regs.h.ah!=0x00)) {
    FREEDOSMEM;
    return(VESA_FAIL);
